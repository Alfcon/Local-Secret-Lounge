...
- **`core/chat_backend.py`**: Resolves whether chat requests should use a local GGUF model, LM Studio, or Ollama.
...
- **`core/chat_engine.py`**: Runs generation through `llama-cpp-python`, LM Studio, or Ollama. Streams partial output to the UI. Uses tokenizer-backed token counting for local GGUF models and a fallback estimate when a tokenizer is unavailable. Cleans model output before the UI receives it.
...
- LM Studio and Ollama requests fall back to prompt-size estimation when no tokenizer is available in-process.
...
2. Open the Anaconda Prompt (Miniconda3) and navigate to the project directory:

```cmd
cd path\to\LocalSecretLounge
